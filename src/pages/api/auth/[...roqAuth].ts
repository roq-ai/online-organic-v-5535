...
export default RoqAuth({
  hooks: {
    ...
    onRegisterSuccess: async ({ user }) => {
      ...
      if (user.roles.data.some((role) => ['Store Manager'].includes(role.name))) {
        const tenant = await roqClient.asSuperAdmin().tenant({ id: user.tenantId });
        const { name: tenantName } = tenant.tenant ?? {};
        seedNumber = await prisma.supermarket.count();
        const mainEntity = await prisma.supermarket.create({
          data: {
            tenant_id: user.tenantId,
            user_id: owner.id,
            name: tenantName,

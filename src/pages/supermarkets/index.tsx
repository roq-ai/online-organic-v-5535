...
type ColumnType = ColumnDef<SupermarketInterface, unknown>;

interface SupermarketListPageProps {
...
export function SupermarketListPage(props: SupermarketListPageProps) {
  const {
    filters = {},
    titleProps = {},
  ...
  const fetcher = useCallback(
    async () =>
      getSupermarkets({
        relations: ['user', 'inventory.count', 'order.count'],
  ...
  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<SupermarketInterface>>(
    () => `/supermarkets?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const handleView = (row: SupermarketInterface) => {
    if (hasAccess('supermarket', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/supermarkets/view/${row.id}`);
    }
  };
      ...
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Flex justifyContent="flex-end">
              <NextLink href={`/supermarkets/view/${record.id}`} passHref legacyBehavior>
                <Button

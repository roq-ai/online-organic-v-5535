...
export function InventoryListPage(props: InventoryListPageProps) {
  ...
  const fetcher = useCallback(
    async () =>
      getInventories({
        relations: ['supermarket', 'discount.count', 'order_item.count'],
  ...
  const columns: ColumnType[] = [
    ...
    { id: 'price', header: 'price', accessorKey: 'price' },
    {
      ...
      cell: ({ row: { original: record } }: any) =>
        record?.expiry_date ? format(parseISO(record?.expiry_date as unknown as string), 'dd-MM-yyyy') : '',
    },
    hasAccess('supermarket', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ? {
          id: 'supermarket',
          header: 'Supermarket',
          ...
          cell: ({ row: { original: record } }: any) => (
            <Link
              ...
              href={`/supermarkets/view/${record.supermarket?.id}`}
            >
              {record.supermarket?.name}
            </Link>
          ),
        }
      : null,
    hasAccess('discount', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ? {
          ...
          cell: ({ row: { original: record } }: any) => record?._count?.discount,
        }
      : null,

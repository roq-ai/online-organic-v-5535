...
const notificationMapping: Record<string, NotificationConfigInterface> = {
  'Order #123 has been placed.create': {
    roles: ['store-manager', 'inventory-manager', 'business-owner'],
    key: 'order_placed',
    tenantPath: [],
    userPath: [],
  },
  'Product XYZ stock has been updated.update': {
    roles: ['store-manager', 'inventory-manager'],
    key: 'inventory_updated',
    tenantPath: [],
    userPath: [],
  },
  'Feedback for Order #123.create': {
    roles: ['store-manager', 'business-owner'],
...
const ownerRoles: string[] = ['store-manager'];
const customerRoles: string[] = ['customer'];
const tenantRoles: string[] = ['store-manager', 'inventory-manager', 'business-owner'];
...
export async function notificationHandlerMiddleware(req: NextApiRequest, entityId: string) {
  ...
  const tenant = await prisma.supermarket.findFirst({

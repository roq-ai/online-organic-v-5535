...
import { getDiscounts, deleteDiscountById } from 'apiSdk/discounts';
import { DiscountInterface } from 'interfaces/discount';

type ColumnType = ColumnDef<DiscountInterface, unknown>;
...
export function DiscountListPage(props: DiscountListPageProps) {
  const {
    filters = {},
    titleProps = {},
  ...
  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<DiscountInterface>>(
    () => `/discounts?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const handleView = (row: DiscountInterface) => {
    if (hasAccess('discount', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/discounts/view/${row.id}`);
    }
  };

  const columns: ColumnType[] = [
    { id: 'discount_percentage', header: 'discount_percentage', accessorKey: 'discount_percentage' },
    {
      id: 'start_date',
      header: 'Start Date',
    ...
    hasAccess('inventory', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Link as={NextLink} onClick={(e) => e.stopPropagation()} href={`/inventories/view/${record.inventory?.id}`}>
      ...
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Flex justifyContent="flex-end">
              <NextLink href={`/discounts/view/${record.id}`} passHref legacyBehavior>
                <Button
              ...
              {hasAccess('discount', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                <NextLink href={`/discounts/edit/${record.id}`} passHref legacyBehavior>
                  <Button
                    onClick={(e) => e.stopPropagation()}
                    mr={2}
                    padding="0rem 0.5rem"
              ...
              {hasAccess('discount', AccessOperationEnum.DELETE, AccessServiceEnum.PROJECT) && (
                <IconButton
                  onClick={(e) => {
                    e.stopPropagation();
    ...
    <Flex direction="column" gap={{ md: 6, base: 7 }} shadow="none">
      <Flex justifyContent={{ md: 'space-between' }} direction={{ base: 'column', md: 'row' }} gap={{ base: '28px' }}>
        <Flex alignItems="center" gap={1}>
          ...
          <AccessInfo entity="discount" />
        </Flex>

        {hasAccess('discount', AccessOperationEnum.CREATE, AccessServiceEnum.PROJECT) && (
          <NextLink href={`/discounts/create`} passHref legacyBehavior>
            <Button
              onClick={(e) => e.stopPropagation()}
              height={'2rem'}

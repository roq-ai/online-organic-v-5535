...
const SidebarContent = ({ onClose, ...rest }: SidebarProps) => {
  ...
  const tenantRoles = ['store-manager', 'inventory-manager', 'business-owner'];
  const isTenantUser = tenantRoles.some((role) => session?.user?.roles?.includes(role));
  const { data, error, isLoading } = useSWR<SupermarketInterface[]>(
    () => (isTenantUser && session?.user?.tenantId ? `/supermarkets` : null),
    () => getSupermarkets({ tenant_id: session?.user?.tenantId }).then(({ data }) => data),
  );
  loading = isLoading;
  ...
  const MockedLinkItems: Array<NavItemPropsInterface> = [
    ...
    {
      name: 'Supermarkets',
      path: isTenantUser ? `/supermarkets/view/${data?.[0]?.id}` : '/supermarkets',
      entity: 'supermarket',
      service: AccessServiceEnum.PROJECT,
      icon: FiShoppingCart,
    },
    { name: 'Discounts', path: '/discounts', entity: 'discount', service: AccessServiceEnum.PROJECT, icon: FiPercent },

...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  const allowed = await prisma.discount
    .withAuthorization({
      roqUserId,
  ...
  switch (req.method) {
    ...
    case 'DELETE':
      return deleteDiscountById();
    default:
      return res.status(405).json({ message: `Method ${req.method} not allowed` });
  }

  async function getDiscountById() {
    const data = await prisma.discount.findFirst(convertQueryToPrismaUtil(req.query, 'discount'));
    return res.status(200).json(data);
  }

  async function updateDiscountById() {
